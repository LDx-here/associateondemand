...
**Last updated:** 2026-07-30 (UTC) — assignment intake → inbox → catalog regression pass (cloud sandbox, PR opened, not yet deployed)
**Workspace:** `/Users/ladaj/Developer/AssociateOnDemand`  
**Branch:** `cursor/phase0-foundation`  
**Remote:** `origin` → `git@github.com:LDx-here/associateondemand.git`

**Pass 58 (2026-07-30, cloud automation — assignment intake/inbox/catalog regression + smoke fixes):** Scheduled cron automation ran on a **cloud sandbox** (no Fly/Vercel CLI, no deploy credentials, no Mac filesystem) rather than the usual local workspace, so this pass verified and hardened the existing assignment-intake → PM-Inbox-review → template-catalog pipeline (already shipped in Passes 48–57) rather than building new surface area, then stopped short of the deploy step it has no credentials for.

- **Full regression sweep first, before touching anything:** `services/api` pytest (124 passed — installed a fresh `.venv` + `requirements.txt`, pytest wasn't pinned there so added it), all 19 `web` `test:*` verify scripts, `tsc --noEmit` (clean), `next build` (clean, 60+ routes), and `scripts/smoke-assignment-e2e.sh` (demo-mode, no-login E2E: matter list → conflict check → intake session autosave → `POST /api/assignments` → PM Inbox status lifecycle Submitted→In progress→Ready for review→Approved → delivered/export → memo export).
- **Found and fixed two real, pre-existing bugs** surfaced by that sweep (not new regressions from this pass):
  1. `test:assessment-docs` was failing — `formatAssessmentOcrForAgents` humanizes `fact_type` keys (e.g. `entry_date` → `entry date`) via `factDisplayLabel` for attorney/LLM readability, but the round-trip test still asserted the raw underscored key. Fixed the assertion to match the intended humanized output (the correct fix — confirmed an identical fix already existed, unmerged, on stale branch `cursor/assignment-intake-workflow-6584`/PR #4 from an earlier cron run; reapplied independently on this pass's branch rather than merging that whole stale PR).
  2. `scripts/smoke-assignment-e2e.sh` **hung indefinitely** when its output was piped (e.g. `| tail`) — the backgrounded `npm run start` → `next-server` process inherited the script's stdout/stderr, so a piping caller (like `tail`, which buffers until EOF) waited forever for that pipe to close, even after the script's own logic finished. `cleanup()` only killed the immediate npm subshell PID, not the reparented `next-server` child. Fixed with `set -m` (own process group) + group-kill + port-based fallback kill (`fuser`/`lsof`) + redirecting server output to a temp log instead of inheriting the pipe. Same fix as an existing unmerged fix on stale branch `cursor/assignment-intake-workflow-6584`; reapplied independently here. After the fix, the full E2E smoke passes cleanly end to end.
- **Three stale open PRs found** (`#2`, `#3`, `#4` — `cursor/assignment-intake-workflow-{3e13,4974,6584}`) from earlier cron runs of this same automation, all targeting `cursor/phase0-foundation` and never merged. They predate Passes 48–57's real shipped work on this branch and are now largely superseded (the assignment-intake/inbox/catalog feature work they proposed already exists on `cursor/phase0-foundation` via later passes) — worth a human decision to close them rather than another agent re-deriving/merging stale diffs. Did not touch or merge them this pass beyond reapplying the two isolated bugfixes above.
- **Not done — flagged, not guessed:** this pass ran in a cloud sandbox with no `flyctl`/`vercel` CLI and no deploy credentials, so `flyctl deploy` / `vercel deploy --prod` from the end-of-pass checklist were **not run**. No API or Fly-side code changed this pass (only a test assertion + a bash smoke script), so there's nothing new to deploy — but the next pass running on the real Mac workspace should still run the full end-of-pass checklist (`pytest` + `next build` + `smoke-production.sh` + `flyctl`/`vercel deploy`) once these two commits land on `cursor/phase0-foundation`, since `smoke-production.sh` (against live Vercel/Fly) genuinely couldn't be exercised from here.

pytest 124 (services/api, all passing, no count change); `test:catalog` (5 launch SKUs / 10 catalog entries), `test:facts`, `test:assessment-docs` (fixed), `test:assignment-transitions`, `test:pm-inbox-options`, `test:google-sheets`, `test:matter-lifecycle-stage`, `test:intake-prefill`, and 11 other `test:*` scripts all green; `tsc --noEmit` clean; `next build` clean; `scripts/smoke-assignment-e2e.sh` full pass (fixed from hanging). PR opened from `cursor/assignment-intake-workflow-0434` → `cursor/phase0-foundation`.

**Next step:** merge this pass's PR (two isolated, low-risk fixes — a stale test assertion and a smoke-script hang) into `cursor/phase0-foundation`, decide whether to close stale PRs #2–#4 as superseded, then resume the still-open items from Pass 57 — confirm imported matters read well on production `/matters`, decide whether Augustine should stay as an empty matter, and rotate the Fly `ANTHROPIC_API_KEY` (returns 401, not a credits problem).
