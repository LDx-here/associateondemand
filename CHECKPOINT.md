...
**Last updated:** 2026-09-07 (UTC) — Pass 58 (cloud automation): assignment intake/inbox/catalog re-verified green; landed 3 CI-blocking fixes that had been rediscovered-and-abandoned on 8+ unmerged PRs

**Pass 58 (2026-09-07, cloud automation, cron):** This automation's mandate each run is "ship assignment intake → inbox workflow → template catalog" — that feature has been complete since Pass 1 (2026-07-02) and iterated through 57 passes since. There was no new intake/inbox/catalog surface left to build, so this pass did a full regression sweep against `cursor/phase0-foundation` and landed three real, previously-undiscovered-on-this-branch fixes:

1. **`test:assessment-docs` was actually failing** — `formatAssessmentOcrForAgents` renders the humanized fact label ("entry date") since the LLM fact-enrichment pass, but the regression test still asserted the pre-humanization raw `fact_type` ("entry_date"). Fixed the assertion.
2. **`smoke-assignment-e2e.sh` could hang for any caller that pipes its output** — only the immediate `npm` child was killed on exit, so an orphaned `next-server` could outlive the script and keep a pipe (e.g. `| tail`) open forever. Now runs the server in its own process group (`set -m`) with a `lsof`-based fallback kill.
3. **13 `react-hooks/set-state-in-effect` / refs-during-render errors across 12 components were failing `npm run lint`** — and CI's `web` job (`.github/workflows/ci.yml`) runs `npm run lint` as a hard gate on every push to `cursor/**` and every PR. This meant **every PR from this repo's `cursor/**` branches has been failing CI red**, unrelated to whatever else that PR shipped.

**The important finding this pass, not a code fix:** all three of the above were not new bugs — they were already fixed, independently, on **8 different unmerged PRs** (`#3` through `#13`) sitting open against `phase0-foundation`, going back to 2026-07-06. Passes 34, 58 (×5 separate branches), 59, 60, 61, and 62 all rediscovered the same stale-assertion / smoke-hang / lint-error trio, fixed it on their own branch, and reported "green" — but **none of those branches were ever merged**, so the base branch stayed broken and the next cron run rediscovered the identical bugs from scratch. That is 6+ cron cycles of duplicated work. Cause: this automation always branches fresh off `phase0-foundation` and opens a new PR (cloud-agent PRs cannot merge themselves); nothing in the loop ever merges a PR back into `phase0-foundation`, so fixes never compound.

**Action needed from La'Dajia (blocking, not a code fix):** there are now **13 open PRs** (`#1`–`#13`) on `github.com/LDx-here/associateondemand`, 11 of which are `cursor/assignment-intake-workflow-*` branches carrying overlapping copies of this same fix. Recommend: review and merge the most recent one with real content (this pass's PR, or `#13`), then close the other 10 as superseded — and/or reconfigure the automation to push follow-up commits onto one persistent PR/branch instead of branching fresh each run, so future passes build on top of merged work instead of re-deriving it. Until a merge happens, every future Mon/Thu run will keep re-finding these same three issues.

**Verified this pass (all green, against `cursor/phase0-foundation` HEAD `ad73f78` + this pass's 3 commits):** `pytest` 136 passed; all 25 `npm run test:*` verification suites pass (catalog, facts, assessment-docs, document-create, matter-link-filter, template-field-maps, onboarding, matter-stage, assignment-transitions, intake-prefill, stripe-pricing, stripe-webhook, firm-knowledge, contacts, google-sheets, aos-intelligence, matter-lifecycle-stage, work-entry, pm-inbox-options, note-fact-extraction, practice-import, practice-pulse, case-state, case-story, work-detection); `npm run lint` 0 errors (20 pre-existing unused-var warnings, unrelated); `tsc --noEmit` clean; `npm run build` green (44 routes); `bash scripts/smoke-assignment-e2e.sh` PASS, completed in ~15s with no hang and no orphaned process; `bash scripts/smoke-production.sh` PASS against live prod (known pre-existing `ANTHROPIC_API_KEY` 401 on Fly still returns template-mode memos — already flagged, not new).

**Not deployed this pass:** no Fly/Vercel CLI or credentials are available in this sandbox, and — more fundamentally — these commits live on a fresh PR branch, not on `phase0-foundation`. Deploying an unmerged branch would just add a 14th disconnected artifact to the pile described above. Deploy should happen from `phase0-foundation` after a human merges a PR.
