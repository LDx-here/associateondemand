...
**Last updated:** 2026-08-10 — Pass 58: CI actually green again (pytest + lint), assignment→inbox→catalog verified end to end

**Pass 58 (2026-08-10, scheduled autonomous pass, cloud sandbox — branch `cursor/assignment-intake-template-catalog-54f3`):** Task was "ship assignment intake → inbox workflow → template catalog; pytest + build + smoke; deploy when green." Those three flows already exist (built across passes 2–58) and `bash scripts/smoke-assignment-e2e.sh` proves them end to end today — create assignment → matter/task/note/inbox row → In progress → Ready for review → Approved → Delivered, all against demo mode. So the actual gap this pass found and closed was that **the "pytest + build" gate itself has been lying since 2026-07-27**: checked GitHub Actions directly (`gh run list`) rather than trusting local runs, and every CI run on `phase0-foundation` since then has failed on *both* jobs, not just some:

- **`api` job:** `requirements.txt` never listed `pytest` — `pip install -r requirements.txt && python -m pytest` has been hitting `No module named pytest` before a single test executes. Every pass since Pass 41 that recorded "pytest 118/124/136 passed" verified against a local venv that happened to already have pytest installed, never against what CI actually runs. Fixed with a new `services/api/requirements-dev.txt` (`-r requirements.txt` + `pytest==8.3.4`); CI workflows now install that instead. Docker/prod image untouched (still installs plain `requirements.txt`).
- **`web` job:** Next 16's `eslint-config-next` promotes the React Compiler hooks lint rules to errors by default, and 13 pre-existing violations across 11 components (`react-hooks/set-state-in-effect`, one real `refs`-during-render) have been failing `npm run lint` — a real CI step that runs before `next build` — since the Next 16 upgrade. Fixed 3 for real (not suppressed): `MatterTasksPanel`'s `tasks` state was a pure unmutated mirror of the `initialTasks` prop (dropped the redundant state), `TemplateFieldForm`'s `sectionPreviews` was synchronously derived with no async step (state+effect → `useMemo`), and `LegalElementsPanel` was mutating a ref during render (`elementsRef.current = elements` moved into a `useEffect` — a genuine correctness hazard, not just a lint nit). The other 8 are legitimate intentional-reset-on-prop-change / one-time-client-hydration patterns (`AssignmentDetailDrawer`, `CaseAssessmentSummary`, `DeliverableTemplateCatalog`, `FirmKnowledgeMap`, `FirmLetterheadSettings`, `KnowledgeMapShell`, `ProceduralTimelinePanel`, `TemplateApplyPanel`) — scoped `eslint-disable-next-line` + one-line rationale each, matching the exhaustive-deps suppression convention already in this codebase, rather than restructuring working components for a newly-stricter rule with zero behavior change on the line.
- **Bonus find:** an earlier abandoned parallel branch (`cursor/assignment-intake-workflow-bf87`, never merged) had already correctly fixed a stale `verify-assessment-documents.mjs` assertion (asserted raw `entry_date` when the code has rendered the human label `"entry date"` since the LLM fact-enrichment pass). Reapplied the same fix here since it was sitting un-merged and the test was failing on this branch too.

**Result:** CI (`gh run list --branch cursor/assignment-intake-template-catalog-54f3`) went green on push — first green run since before this log's Pass 41, as far as the run history shows. 136 pytest; 0 eslint errors (19 pre-existing unused-var/exhaustive-deps warnings untouched, same class already flagged as non-blocking debt); `next build` 44 routes; all 24 `npm run test:*` verify scripts; `smoke-production.sh` (live prod — web auth gate, API health, PM dispatch still `llm: template`, same pre-existing Anthropic-key blocker as before); `smoke-assignment-e2e.sh` full lifecycle in local demo mode.

**Could not deploy from this session:** this pass ran in a cloud sandbox with no `flyctl`/`vercel` CLI installed and no deploy credentials — a real environment difference from the laptop-based continuation-agent runbook, not a shortcut taken. No app code changed (only CI config, lint fixes, and a test assertion), so Fly/Vercel production is unaffected either way; opened a PR for review/merge so the next session with deploy access (or the existing Vercel GitHub integration, if connected) can ship it.
...
1. **Merge/deploy Pass 58** — review + merge PR from `cursor/assignment-intake-template-catalog-54f3` into `cursor/phase0-foundation`, then `flyctl deploy -a associateondemand-api` + `vercel deploy --prod` (only CI config/lint/test-assertion changes — no runtime behavior change expected, but re-run `smoke-production.sh` post-deploy anyway). Confirm the next scheduled-health / CI run off `phase0-foundation` itself is green, not just this branch.
