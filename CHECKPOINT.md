...
**Last updated:** 2026-08-06 (UTC) — Cloud automation pass, catalog billing-note fix (see below)
**Workspace:** `/Users/ladaj/Developer/AssociateOnDemand`  
**Branch:** `cursor/phase0-foundation`  
**Remote:** `origin` → `git@github.com:LDx-here/associateondemand.git`

**Pass 58 (2026-08-06, Cursor cloud automation — assignment intake / inbox / template catalog):** Ran the scheduled autonomous-agent-pass with no deploy credentials available in the cloud sandbox (no `flyctl`/`vercel` CLI, no Google/Airtable/Anthropic secrets) — scope was code, tests, and a PR, not a live Fly/Vercel deploy. Per the 2026-08-03 handoff brief (`docs/runbooks/project-debt-and-cleanup.md`), verified the assignment-intake → inbox → template-catalog pipeline before changing anything: `bash scripts/smoke-assignment-e2e.sh` (local demo mode) passed clean end to end — matter link, conflict check, intake session autosave, assignment creation, full inbox lifecycle (In progress → Ready for review → Approved), delivered/export event, memo export. `bash scripts/smoke-production.sh` also passed against the live URLs (still shows the known Fly `ANTHROPIC_API_KEY` 401 from Pass 57 — attorney-side, unchanged).

Found one real bug while reading the catalog code the debt brief's root cause #2 predicts (marketplace framing bleeding into her own solo-practice screens): `deliverable-catalog.ts`'s `billingNoteForPartnerFirm()` was baked into every catalog entry's `pricing.note` and rendered on **`/assignments/new`** and **`/templates`** — her own internal system — as *"Quoted flat fee for partner firm — invoiced off-platform. RMV does not pay through this dashboard."* She is not a partner firm paying RMV; she is RMV, drafting for her own matters. This is the same category of bug Pass 50 fixed elsewhere (dashboard quick actions, inbox subtitle, intake disclaimer) and the Aug 3 dashboard pass fixed again (`af1eaa1`) — this file was simply missed both times, likely because the catalog is genuinely shared with the real external `/partner/submit` funnel where the partner language is correct.

Fixed with the same `isPartnerSubmission` pattern already established in `lib/intake-disclaimer.ts`: new `billingNoteFor(entry, isPartnerSubmission)` returns honest "reference pricing, not billed through this dashboard" copy on the internal screens and keeps the accurate partner-invoicing copy on `/partner/submit` (unchanged there). Split `custom-motion`'s setup-surcharge advisory out of the partner note into its own `pricing.advisory` field so both contexts keep that information instead of the internal view silently losing it. `Settings` page's partner-billing section (`billingNoteForPartnerFirm()`) was deliberately left untouched — it is specifically the "how partner firms pay" explainer and Pass 50 already confirmed that framing is correct there. Added a regression guard to `test:catalog` asserting the internal note never contains "partner firm" and the partner note never loses its invoicing language.

Also found and fixed a stale test assertion in `test:assessment-docs` (pre-existing failure, not introduced this pass — confirmed by reproducing it on `5cd572f` before touching anything): `formatAssessmentOcrForAgents()` renders the human-readable fact label (`"entry date"`) rather than the raw snake_case `fact_type` (`"entry_date"`) since the LLM fact-enrichment pass, but the regex assertion was never updated, so the test had been silently broken — exactly the "green pipeline, no coverage" failure mode the handoff brief warns about. Updated the assertion to match current (correct) behavior plus a `doesNotMatch` guard against regressing back to raw fact_type.

**Verification honesty (per the handoff brief's rule):** everything above was checked against demo-mode seed data and the public production smoke endpoints only. This cloud sandbox has no Google Sheets/Airtable/Anthropic/Fly/Vercel credentials, so **I could not** open the change against one of her real imported matters (Makhammad/Limbu/Akyaa/Hammond/Viazovikova/Konst) on `aod-next.vercel.app`, and could not run `flyctl deploy`/`vercel deploy --prod` from here — pushed to `cursor/assignment-intake-workflow-bf87` and opened a PR into `cursor/phase0-foundation` instead of deploying directly. Whoever merges should do the real-matter check and redeploy.

Gates: pytest 124 passed; `tsc --noEmit` clean; `next build` clean (44 routes); `test:catalog` (with new regression guard), `test:assessment-docs` (fixed), and the full `test:*` suite (facts, contacts, google-sheets, matter-lifecycle-stage, work-entry, pm-inbox-options, practice-import, practice-pulse, case-state, etc.) all green; `smoke-assignment-e2e.sh` and `smoke-production.sh` both PASS.
