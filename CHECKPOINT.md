...
**Last updated:** 2026-08-20 (UTC) — Pass 59: CI actually merged this time (pytest + lint + stale test + smoke hang) — see branch-backlog warning below
...
1. **Merge Pass 59's PR into `cursor/phase0-foundation`, then close the redundant stale PRs** (#4, #6, #7, #8 are superseded by this branch; #2/#3/#5 need a quick skim first) — this is the single highest-leverage action available: every scheduled run since 2026-07-02 has been re-diagnosing the same CI/lint/test bugs from the same stale base because nothing ever merges. Then `flyctl deploy -a associateondemand-api` + `vercel deploy --prod` (CI-config/lint/test-only changes, no runtime behavior change, but re-run `smoke-production.sh` after anyway) and confirm the next scheduled-health run off `phase0-foundation` itself goes green.
