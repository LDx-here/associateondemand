...
**Last updated:** 2026-09-10 (UTC) — Pass 62: CI was actually broken for 3+ weeks; fixed on the real base branch

**Pass 62 (2026-09-10, assignment intake → inbox → catalog — CI actually fixed this time, plus a merge-backlog flag):** Ran the runbook's assignment-intake/inbox/catalog charter. Before touching features, set up this sandbox from scratch (no node_modules, no Python deps existed) and ran the real gates — and found `cursor/phase0-foundation` itself has been failing **every CI run since 2026-08-17** (both `Scheduled health` and every push), not just the 13 stale unmerged PR branches that kept re-discovering and re-"fixing" the same three regressions without ever landing on this branch:

1. **API job: 100% broken.** `pip install -r requirements.txt` never included pytest, so `python -m pytest tests/ -q` failed with `No module named pytest` before running a single test — every single CI run, for weeks. Fixed with a new `services/api/requirements-dev.txt` (`-r requirements.txt` + `pytest` + `pytest-asyncio`); both `ci.yml` and `scheduled-health.yml` now install from it. 136 pytest now actually run and pass in CI, not just locally.
2. **Web job: `npm run lint` broken.** A newer `eslint-plugin-react-hooks` (already pinned in `package-lock.json`, so `npm ci` picks it up regardless of when you last ran `npm install`) added a stricter `react-hooks/set-state-in-effect` rule that 12 components tripped — 13 genuine "sync external state on mount" effects (fetch-then-setState, SSR-guarded localStorage/URL reads, reset-on-prop-change), fixed with scoped `eslint-disable-next-line` + reason, same precedent already used in this codebase for `exhaustive-deps`. One real bug in the batch: `LegalElementsPanel.tsx` wrote `elementsRef.current = elements` directly in the render body every render — moved into a `useEffect`, the standard-safe fix.
3. **`smoke-assignment-e2e.sh` hangs forever when piped** — reproduced live, not assumed: `bash scripts/smoke-assignment-e2e.sh | tail -80` hung 7+ minutes even though the script's own assignment-intake → inbox-lifecycle → export checks all passed and it exited 0. The backgrounded `npm run build && npm run start` subshell inherited the caller's stdout pipe; `next-server` outlives the PID the cleanup trap kills, so the orphaned process keeps that pipe's write end open forever and the caller never sees EOF. Fixed by giving the server its own log file instead of sharing our stdout, plus a harder cleanup (kill by PID, sweep anything on `$PORT`, `pkill -f next-server`). Verified clean (~15-18s, zero leftover processes) across five repeated runs, including the exact piped-caller shape that used to hang.
4. **`test:assessment-docs` stale assertion** — `formatAssessmentOcrForAgents` humanizes `fact_type` for readability (`entry_date` → `entry date`) but the test still regex-matched the raw key. Fixed the assertion to match the actual (correct) human-readable output.

All four are exactly the categories 6+ of the 13 stale PRs (`#4` through `#14`, July 2 → Sept 7) independently rediscovered and "fixed" on their own branches — but every one of those branches forked from an old point in `phase0-foundation`'s history (before Five Anchors / case journey / billing shipped) and none were ever merged, so none of their fixes ever reached the branch real work continues on. This pass fixed all four directly on current `phase0-foundation` tip instead of on a side branch, and re-verified the full assignment intake → inbox lifecycle → export flow end to end (matter detail nav, conflict check, intake session autosave, create assignment, full status lifecycle Submitted→In progress→Ready for review→Approved, delivered event, memo export) still passes against the live demo seed.

**Flagging for attorney/repo-owner attention — not fixed, cannot fix from here:** `cursor/phase0-foundation` has **13 open, unmerged PRs** stacked against it (`#2` through `#14`, oldest from 2026-07-02), every one titled around this same assignment-intake/inbox/catalog charter, because this automation can open PRs but is not permitted to merge them. Each cron run branches fresh from `phase0-foundation`, can't see fixes sitting in a prior run's unmerged PR, and — since CI has been silently broken this whole time — has no green build to bounce off of, so the same lint/pytest/smoke regressions keep reappearing across passes that never talk to each other. Recommend: merge this PR first (it makes CI green again on the target branch, which is what every later PR needs to build on), then close or re-base the other 12 as superseded — most predate the Five Anchors/case-journey/billing features and would delete them if merged as-is. Longer term, consider pointing this automation's PRs at a branch someone actually reviews on a cadence, or granting a merge path, so this stops recurring.

pytest 136; `npm run lint` 0 errors; `npm run build` (all 78 routes); `npm run test:*` (34 verify scripts, all pass — `test:assessment-docs` fixed, everything else was already green); `bash scripts/smoke-assignment-e2e.sh` (piped and unpiped, both clean); `bash scripts/smoke-production.sh` PASS against the current live deploy. **Not deployed:** this sandbox has no Vercel/Fly CLI or credentials, so Fly/Vercel deploys from this pass are not possible here — production still serves Pass 61's commit (`bcd54be`) exactly as `smoke-production.sh` confirmed above. Pushed to `cursor/assignment-intake-workflow-8f42`; PR opened against `cursor/phase0-foundation`.
...
0. **Merge the PR backlog on `cursor/phase0-foundation`** — 13 open PRs (`#2`–`#14`) are stacked against it, none merged; this pass's PR (fixes CI + smoke hang, see Pass 62 above) should land first since everything else needs a green base branch to build on. Close/re-base the other 12 as superseded (most predate Five Anchors/case-journey/billing and would regress the app if merged as-is).
