...
**Last updated:** 2026-08-27 (UTC) — Pass 58: assignment intake → inbox → catalog regression pass (CI green again)
...
1. **Merge the PR backlog** — `#2`–`#10` are all open against `cursor/phase0-foundation`; this pass's PR is self-contained and mergeable on its own, but the fastest way to stop the cron from re-diagnosing the same CI bugs is for a human with repo-write access to merge one consolidated PR (this one, or #10) and close the rest without merging. Then `flyctl deploy -a associateondemand-api` / `vercel deploy --prod` (no deploy CLI in this sandbox).
