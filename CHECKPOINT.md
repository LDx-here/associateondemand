...
**Last updated:** 2026-08-03 (UTC) — Pass 58 regression pass (assignment intake/inbox/catalog), CI lint fixed
**Workspace:** `/Users/ladaj/Developer/AssociateOnDemand`  
**Branch:** `cursor/phase0-foundation` (this pass shipped on `cursor/assignment-intake-workflow-catalog-a24d`, PR pending)
**Remote:** `origin` → `git@github.com:LDx-here/associateondemand.git`

**Pass 58 (2026-08-03, scheduled cron automation — assignment intake/inbox/catalog regression pass):** Ran the `docs/runbooks/autonomous-agent-pass.md` protocol. Found this exact cron task had already fired several times without ever merging: four stale open PRs (`cursor/assignment-intake-workflow-{0434,3e13,4974,6584}`) sat unmerged against `cursor/phase0-foundation`, so each new run re-diagnosed the same ground. The most recent one (`-0434`, "Pass 58") had already found and fixed two real bugs against the exact same base commit this pass started from — reapplied both here since they were still live:
1. `scripts/smoke-assignment-e2e.sh` could hang forever for any caller piping its output (e.g. `| tail`) — the background `npm run start` forks `next-server`, which can get reparented past the cleanup trap's `kill "$SERVER_PID"` and keeps holding the script's inherited stdout/stderr pipe open. Fixed with `set -m` (own process group) + group-kill + port-based fallback kill + server log redirected to a temp file instead of inherited fds. Verified: `timeout 60 bash scripts/smoke-assignment-e2e.sh | tail -5` now exits cleanly instead of hanging.
2. `web/scripts/verify-assessment-documents.mjs` asserted the raw `fact_type` (`entry_date`) in `formatAssessmentOcrForAgents` output, but that function renders facts through `factDisplayLabel`, which humanizes the key (`entry date`) for attorney/LLM readability — test was failing pre-existing. Fixed the assertion.

**New finding this pass, not in any prior PR — `npm run lint` was fully red (13 errors) on `cursor/phase0-foundation`'s tip**, would have failed `ci.yml`'s `web` job on the next push. Root cause: `eslint-config-next` is on v16, which bundles the React Compiler's `react-hooks/set-state-in-effect` (and refs-during-render) lint rules as hard errors — these didn't exist when most of this code was written, so debt had piled up quietly. Every error lived inside the three areas this pass's brief named — assignment intake/inbox (`AssignmentDetailDrawer`), and template catalog (`TemplateApplyPanel`, `TemplateFieldForm`, `DeliverableTemplateCatalog`) — plus the matter workbench (`CaseAssessmentSummary`, `LegalElementsPanel` — also had a genuine "refs during render" error from `elementsRef.current = elements` assigned directly in the component body, `MatterTasksPanel`, `ProceduralTimelinePanel`, `FirmKnowledgeMap`, `FirmLetterheadSettings`, `KnowledgeMapShell`). Fixed all 13 using the two patterns already established in this codebase (see Pass 7's `AgentResultPanel`/`EditableOutputMemo` fix): (a) the React-recommended "adjust state during render when a prop/key changes" pattern instead of an effect, or convert to `useMemo` when the value was purely derived; (b) for genuine external-system reads that can't move to render time without a hydration mismatch or aren't available server-side (browser `location.hash`, `localStorage`), kept the effect with a justified `eslint-disable-next-line` comment. Mount-time async fetches whose callback synchronously calls `setState` before their first `await` (`loadPreview`, `seedCoreElements`, the template-catalog `refresh`) are now invoked via `Promise.resolve().then(fn)` so the synchronous prefix runs as a microtask outside the effect's own synchronous execution. `npm run lint` is now **0 errors** (18 pre-existing, unrelated `no-unused-vars` warnings untouched).

Verified: `services/api` pytest 124/124; all 20 `web` `test:*` verify scripts green (`test:catalog`, `test:facts`, `test:assessment-docs`, `test:matter-stage`, `test:assignment-transitions`, `test:pm-inbox-options`, etc.); `tsc --noEmit` clean; `next build` clean; `npm run lint` clean; `bash scripts/smoke-assignment-e2e.sh` — full local demo-mode intake → matter → conflict-check → create assignment → inbox lifecycle (In progress → Ready for review → Approved) → delivered/export round-trip, PASS; `bash scripts/smoke-production.sh` — PASS against live `https://aod-next.vercel.app` / `https://associateondemand-api.fly.dev` (auth gate, API health, PM research dispatch all green; the pre-existing `llm: template` / Anthropic 401 warning is the already-tracked Fly key issue below, unrelated to this pass).

**Not done this pass — no deploy:** this Cloud Agent sandbox has no `flyctl`/`vercel` CLI or credentials (same blocker Pass 7 hit on 2026-07-06), so `flyctl deploy` / `vercel deploy --prod` were not run. Nothing in this pass touched `services/api` or required a Fly redeploy — it's a `web`-only lint/test fix, and `web/` is on Vercel's auto-deploy-from-branch pipeline once the PR merges (confirm before assuming prod picked it up, since this pass could not verify a fresh Vercel deployment directly). The other three stale PRs (`-3e13`, `-4974`, `-6584`) were not merged or closed — that's a merge-workflow decision for La'Dajia, not something to do unprompted from a read-only `gh` CLI.

**Anthropic credits check (2026-07-29, still open):** `ANTHROPIC_API_KEY` is present/Deployed on `associateondemand-api`, but live PM research smoke still returns `llm: template`. Fly logs: Anthropic **401 invalid x-api-key** (not a credit/billing error). Credits alone do not unblock — rotate/replace the Fly secret with a valid key (`fly secrets set ANTHROPIC_API_KEY=… -a associateondemand-api`). `AOD_AOS_USE_API` intentionally unset (library FILL default; smart paste extract needs only a valid Anthropic key).
...
0. **Merge or close the backlog of stale `cursor/assignment-intake-workflow-*` PRs** — [`#2`](https://github.com/LDx-here/associateondemand/pull/2), [`#3`](https://github.com/LDx-here/associateondemand/pull/3), [`#4`](https://github.com/LDx-here/associateondemand/pull/4), [`#5`](https://github.com/LDx-here/associateondemand/pull/5), plus this pass's PR — this cron automation has been firing every run without anything landing on `cursor/phase0-foundation`, so real fixes (this pass's CI-lint fix, Pass 58/`-0434`'s smoke-hang fix) sit unmerged and get silently re-discovered each time. Recommend merging the newest (this pass) and closing the older three as superseded once reviewed, so the next scheduled run starts from a base that actually has the fixes.
