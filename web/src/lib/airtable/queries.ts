 ...
 *
 * Moved from the flat `web/src/lib/airtable-queries.ts`. Imports the
 * BUILD_SPEC-aligned table names from `./fields` and continues to read /
 * write against the Phase 1 base shape via `LEGACY_FIELDS`. Once the
 * live base is migrated to BUILD_SPEC §2 column names this module will
 * switch its field references to `SPEC_FIELDS`.
 */
...
import { FIELDS, TABLES } from "./fields";
...
export async function completeTaskInAirtable(taskId: string): Promise<Task | null> {
  const rec = await airtablePatch(TABLES.tasks, taskId, { [FIELDS.tasks.status]: "Done" });
  ...
  try {
    const matterRec = await airtableGetRecord<RawFields>(TABLES.matters, recordId);
