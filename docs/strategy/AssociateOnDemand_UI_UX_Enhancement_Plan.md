...
The attorney portal, matter workbench, template catalog, and assignment intake form at `/assignments/new` are already live in `web/` (see [`CHECKPOINT.md`](CHECKPOINT.md)). The PM Inbox Kanban, document upload with OCR into Airtable **Documents**, and facts **Notes** are shipped; chat-centric intake is not. UX work should enhance what exists—especially intake autosave, catalog pricing display, and portal polish—not assume a greenfield rewrite.
...
The chat interface will complement—not replace—the existing assignment intake form, serving as a conversational layer that pre-fills structured fields before submit.

*   **Intuitive Onboarding:** A guided, conversational chat flow will **enhance** assignment intake with conversational pre-fill (deliverable type, tier, facts, attachments). The existing form at `/assignments/new` remains the source of truth until a chat MVP is validated in production. This chat will intelligently collect case details, service requirements (e.g., Immigration Brief Support), and attorney preferences, then populate the form for attorney review and submit.
...
1.  **Refining the `web/` codebase** to add chat-enhanced intake that wraps and pre-fills `/assignments/new`, keeping the form as the submission source of truth for v1.
...
[3] AssociateOnDemand Project DNA. (2026). *The Vision and Context*. In-repo: [`AssociateOnDemand Project DNA: The Vision and Context.md`](AssociateOnDemand%20Project%20DNA%3A%20The%20Vision%20and%20Context.md). Harvey-style UX direction supplemented by user-provided notes (July 2026 strategic planning session).
