...
As of July 2026, the production firm OS is live: assignment intake at `/assignments/new`, a PM Inbox Kanban review workflow, specialist agents with OCR ingestion, and DOCX export are all shipped in the `web/` Next.js app (see [`CHECKPOINT.md`](CHECKPOINT.md)). Payments, engagement letters, and chat-centric intake are not yet built—the workflow today runs from form submit → Airtable matter/task/note creation → inbox review → export. This analysis describes the B2B overflow model on top of that baseline, not a greenfield platform.
...
**Catalog alignment:** In the live template catalog (`web/src/lib/deliverable-catalog.ts`), **Motion / Short Filing** maps to the `custom-motion` SKU (Custom tier — scoped after intake). **Hearing Packet / Exhibit Organization** is not yet a catalog entry; treat it as a planned/future SKU until added to the catalog. Pricing rows above remain valid market targets; annotate quotes against catalog IDs when selling.
...
lawyer-reviewed work product that attorneys can actually use, rather than just AI drafts (see [`AssociateOnDemand Project DNA: The Vision and Context.md`](AssociateOnDemand%20Project%20DNA%3A%20The%20Vision%20and%20Context.md) and user-provided pricing notes from the July 2026 strategic planning session).
...
2.  **Upload Sample/Template (if available):**
    *   **Platform Role:** The UI will provide a secure document upload feature. These documents (e.g., previous briefs, templates) will be ingested by the "Strong Reader" (OCR) module. Extracted text is stored in the **Documents** table in Airtable, linked to the specific matter; the attorney's facts packet and agent memos land in **Notes**.
...
6.  **Drafting Process:**
    ...
    *   **Quality Control:** The platform will facilitate human review by the supervising attorney, ensuring attorney judgment, citation verification, and local practice sense are applied.
...
This positioning emphasizes that "AI can draft. We deliver lawyer-reviewed work product attorneys can actually use" (user-provided positioning notes, July 2026 strategic planning session).
...
[3] AssociateOnDemand Project DNA. (2026). *The Vision and Context*. In-repo: [`AssociateOnDemand Project DNA: The Vision and Context.md`](AssociateOnDemand%20Project%20DNA%3A%20The%20Vision%20and%20Context.md). Positioning and pricing context supplemented by user-provided notes (July 2026 strategic planning session).
