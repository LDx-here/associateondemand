...
### [2026-08-24] CHECKPOINT: Pass 59 (Five Anchors intake, manual-first) — new Case anchors tab renders the blueprint's five-anchor intake as 19 typed questions with no model in the loop; fifth anchor (uncertainty and gaps) tracked as its own readiness flag with an amber callout while empty; persists as one typed Anchors Note per matter via findAnchors on both Sheets and dormant Airtable backends; autosaves on debounce and blur; fixed structured payload notes leaking into the case-story timeline as bare type words or raw JSON (new lib/note-kinds.ts readableNotes(), applied in buildStoryTimeline and the activity feed); confirmed a pre-existing hydration error on the matter page from readInitialTabFromUrl reading window.location, not introduced here; verified full round-trip against the live Google Sheet on AOD-1008 with demoMode false; 136 pytest, test:five-anchors + 5 web suites, tsc, next build --webpack, Vercel prod + smoke; commit 133c45f not yet pushed (push blocked by permission classifier)

### [2026-09-07] CHECKPOINT: Pass 60 (case journey) — two journeys transcribed from her own walkthroughs (removal defense 12 steps NTA→decision; injury claim 8 steps injury→disbursement incl. Medicaid lien accounting); every step carries awaiting you/them because half of practice is knowing whose move it is; manual step selection with posture only as the opening guess; persists as a typed Journey note; verified live on AOD-1008 (step 9/12 waiting on IH date) and AOD-1007 (step 5/8 waiting on settlement offer)
### [2026-09-07] CHECKPOINT: Pass 61 (billing loop) — new Billing tab; invoice lines from logged time plus manual fee/expense lines; billedNoteIds prevents double-billing; integer cents and local due dates; plain-text client email copied not sent; Stripe payment link per invoice via Checkout Session with {matterId,invoiceId} metadata and webhook routing to markInvoicePaid; hourly rate starts unset and blocks invoicing until entered; Stripe keys still unset everywhere so the link path returns a clear 503; verified on live Sheet on AOD-1007 then removed the test invoice

### [2026-09-13] CHECKPOINT: Pass 62 (site cleanup) — Draft Review panel identified as the session-only AI command console, now closed by default and relabeled AI drafts; AI-status banner moved to Settings; fixed dashboard subtitle showing marketplace copy to gmail.com users; New matter replaces New assignment; practice-first nav and tagline; removed AI KPI row, Firm Memory nag, and marketplace onboarding modal; open-matters card now counts open matters; matter page loses drafting strip, overflow next-action, facts chip, and duplicate posture/summary; stage chip only with assignments; instructions collapsed; New matter case types fixed (had no Personal Injury, never matched a journey); Documents empty state names the Drive folder and file count; hydration error no longer reproduces but cause not isolated and Pass 59 diagnosis corrected; flagged test matters AOD-1001/1002 for her decision; deeper UX routed to a planning agent

### [2026-09-13] CHECKPOINT: Pass 63 (push failure notices) — traced to three standing causes predating recent work: CI api job and scheduled health check never installed pytest (added requirements-dev.txt; 136 pass in a clean 3.12 venv); CI web lint failed on 13 React Compiler rule errors in older components (set to warn, still printed, refactor is open debt; lint exits 0, Turbopack build passes); Vercel recovermyvalue-site project built every cursor/ push from a repo root with no package.json — its production deploys from main, so rather than disconnecting, a root vercel.json disables Git deployments for cursor/** (aod-next reads web/ only); archived test matters AOD-1001 and AOD-1002 at her direction; Stripe secret match still unverified pending her test event
