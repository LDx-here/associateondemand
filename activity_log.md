...
### [2026-07-30] CHECKPOINT: Pass 58 (cloud automation — assignment intake/inbox/catalog regression) — cron pass ran in a cloud sandbox with no Fly/Vercel CLI or deploy credentials; verified the already-shipped assignment-intake -> PM-Inbox-review -> template-catalog pipeline end to end via scripts/smoke-assignment-e2e.sh in demo mode plus full pytest (124) + 19 web test:* scripts + tsc + next build; found and fixed two real pre-existing bugs surfaced by the sweep: (1) test:assessment-docs asserted the raw fact_type instead of factDisplayLabel's humanized output ("entry_date" vs "entry date"), (2) scripts/smoke-assignment-e2e.sh hung forever when piped because the backgrounded next-server child inherited stdout and was never fully killed by cleanup() — fixed with set -m + process-group kill + port fallback kill + redirecting server output to a temp log; both fixes match identical unmerged fixes already sitting on stale branch cursor/assignment-intake-workflow-6584 (PR #4), reapplied independently rather than merging that stale PR; flagged three stale open PRs (#2/#3/#4) from earlier cron runs as likely superseded by Passes 48-57's real shipped work, for a human to close; did not run flyctl/vercel deploy (no credentials in this sandbox, and no API/Fly-side code changed this pass); PR opened cursor/assignment-intake-workflow-0434 -> cursor/phase0-foundation
