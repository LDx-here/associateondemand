...
### [2026-08-06] CHECKPOINT: Pass 58 (Cursor cloud automation, assignment intake/inbox/catalog) — no deploy credentials in the cloud sandbox (no flyctl/vercel/Google/Airtable/Anthropic secrets), so scope was code+tests+PR, not a live deploy. Verified smoke-assignment-e2e.sh (demo) and smoke-production.sh (live URLs) both PASS before touching anything. Found deliverable-catalog.ts's billingNoteForPartnerFirm() rendering "Quoted flat fee for partner firm — invoiced off-platform. RMV does not pay through this dashboard." on her own internal /assignments/new and /templates screens — the same marketplace-framing bug Pass 50 and the Aug 3 dashboard pass fixed elsewhere, missed in this shared catalog file. Fixed with billingNoteFor(entry, isPartnerSubmission), mirroring intake-disclaimer.ts's existing pattern; /partner/submit keeps its correct partner-invoicing copy; Settings' partner-billing section left untouched (correctly partner-facing per Pass 50). Also fixed a stale test:assessment-docs assertion (expected raw fact_type "entry_date"; code now correctly renders the human label "entry date" since the fact-enrichment pass — test was silently broken, not the code). pytest 124; tsc clean; next build clean; full test:* suite green; both smoke scripts PASS. Pushed to cursor/assignment-intake-workflow-bf87, PR opened into cursor/phase0-foundation — could not verify against her real Sheets matters or redeploy from this sandbox; flagged for whoever merges.
