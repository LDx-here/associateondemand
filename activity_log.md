...
### [2026-08-10] CHECKPOINT: Pass 58 (scheduled autonomous pass, cloud sandbox) — verified assignment intake → inbox workflow → template catalog end to end (bash scripts/smoke-assignment-e2e.sh: create assignment → In progress → Ready for review → Approved → Delivered, all against demo mode locally) and found GitHub Actions CI has been red on every run since 2026-07-27 on both jobs, not just this branch — api job: requirements.txt never listed pytest, so pytest never actually ran in CI despite every pass since then recording a green pytest count from a local venv; web job: Next 16's eslint-config-next promotes React Compiler hooks rules to errors, and 13 pre-existing violations across 11 components had been failing `npm run lint` (a real CI step) the whole time. Fixed both: new services/api/requirements-dev.txt for CI's pytest install; 3 real issues fixed properly (ref mutated during render in LegalElementsPanel, two components deriving/mirroring state that should not have been state at all); 8 legitimate intentional-reset patterns given scoped eslint-disable comments with rationale. Also reapplied a stale-assertion test fix (verify-assessment-documents.mjs entry_date vs "entry date") that an earlier abandoned parallel branch (cursor/assignment-intake-workflow-bf87) had already made correctly but never merged to phase0-foundation. CI now green on push for the first time since before this log's Pass 41. Could not run `flyctl deploy` / `vercel deploy --prod` from this cloud sandbox — no deploy CLI or credentials available in this environment (a real difference from the laptop-based continuation-agent runbook, not a shortcut); PR opened for review/merge and deploy instead. 136 pytest, 0 eslint errors, next build (44 routes), all 24 web verify: scripts, smoke-production.sh, smoke-assignment-e2e.sh — all PASS.
