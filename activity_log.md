...
### [2026-08-20] CHECKPOINT: Pass 59 (scheduled autonomous pass, cloud sandbox) — verified assignment intake → inbox → template catalog end to end via smoke-assignment-e2e.sh (PASS, demo mode). Real finding: this same cron automation had fired ~6 times since 2026-07-02 and every run opened a new unmerged PR (#2-#8) against phase0-foundation, so four separate runs (PRs #4/#6/#7/#8) independently rediscovered and fixed the same three bugs — pytest missing from requirements.txt (CI's pytest job never actually ran a test since before 2026-07-27), 13 eslint react-hooks/set-state-in-effect errors from the Next 16 upgrade (npm run lint fully red), and a stale verify-assessment-documents.mjs assertion (raw fact_type vs the humanized label the code has rendered since the LLM fact-enrichment pass). Consolidated by cherry-picking the verified commits from those branches instead of refixing from scratch, then found and fixed a fourth bug the newer PRs missed reapplying from the oldest stale branch: scripts/smoke-assignment-e2e.sh hangs forever when piped (orphaned next-server keeps the pipe open past cleanup) — reproduced live, fixed with PR #4's set -m process-group kill. New live finding, not yet fixed (no Fly credentials in this sandbox): associateondemand-api /health/deps reports postgres down (degraded status) — does not block assignment/inbox/catalog (those run on Airtable/Sheets) but is a real current outage. pytest 136; npm run lint 0 errors; tsc clean; next build 44 routes; all 24 test:* scripts; smoke-assignment-e2e.sh PASS; smoke-production.sh web gate + API liveness PASS, deps degraded. No flyctl/vercel in this sandbox — could not deploy; flagged for whoever merges to also close the now-superseded PRs #4/#6/#7/#8.
### [2026-08-24] CHECKPOINT: Pass 60 (scheduled autonomous pass, cloud sandbox) — same cron task as Pass 59, fired against the same stale phase0-foundation tip because PR #9 (Pass 59 consolidation) was still open/unmerged 4 days later. Instead of re-diagnosing the same CI/lint/test bugs a ninth time, git-merged PR #9's branch directly (clean, no conflicts) and re-verified every gate from a clean install: 136 pytest passed; npm run lint 0 errors; tsc clean; next build clean; all 24 test:* verify scripts green; smoke-assignment-e2e.sh PASS (intake session -> assignment -> matter/task/note/inbox row -> lifecycle -> export, demo mode, no hang); smoke-production.sh web gate PASS, live curl confirms aod-next.vercel.app healthy (demoMode false); Fly API liveness ok but /health/deps still degraded/postgres:false, same outage Pass 59 found 4 days ago, confirmed still live not stale. No flyctl/vercel in this sandbox, could not deploy. Nine open draft PRs (#2-#9) now sit unmerged against phase0-foundation from repeated cron runs of this same task; flagged again for repo-write access to merge this PR and close the rest.
